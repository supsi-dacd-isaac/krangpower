...
Krang has a graph attributes that returns a networkx_ Graph. The nodes of this graph are constituted by the buses; the
...
Krang instance and two arguments, busfun and edgefun, that are functions meant to be applied to every :code:`'el'[0]` of the
edges and to every :code:`'bus'` in the nodes (these function can also be None).
...
a :code:`Krang` instance and returns the property Voltages() of both 'bus' at buses and 'el'[0] at edges.
More specialized built-in :code:`GraphView` can and will be added in the future to krangpower.gv.
